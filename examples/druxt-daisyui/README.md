...
- A themed all-in-one `druxt-site` integration (the "fastest path". For
  comparison, see `druxt-tailwind`/`druxt-bootstrapvue`'s bespoke-module
  approach).
...
- One store action reachable three ways (drag, buttons, keyboard). A
  concrete example of building an accessible interaction on top of one
  source of truth.
