...
A few small wrapper components fix real gaps in the plain
`druxt-site` render, modeled on `sites/umami.demo.druxtjs.org`'s own
(much larger) theme and kept as minimal as each block actually needs:

- `components/druxt/site/Umami.vue` pins the region order. Drupal's
  block-placement API gives regions with no visual order (see
  `DruxtSite.vue`'s own docblock). Without
...
- `components/druxt/entity/taxonomy_term/Default.vue` supplies a missing
  display mode. The Umami demo
  ...
  a real gap in the demo profile, since taxonomy terms aren't normally
...
- `components/druxt/block/SystemBrandingBlockUmami.vue` and
  `SearchFormBlockUmami.vue` provide the Umami theme's real logo and a
  plain `GET /en/search/node` form (Drupal core search, already enabled
  on this backend). Both work end to end.
- `components/druxt/block/SystemMessagesBlockUmami.vue`,
  `HelpBlockUmami.vue`, and `LocalTasksBlockUmami.vue` all three
  ...
  answer, not a fake version of something this app can't actually
  do.
