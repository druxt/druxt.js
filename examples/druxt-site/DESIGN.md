...
`mode`, field `errors`) do the styling.
...
- The Umami theme's own logo.svg, included with the Drupal profile, is
...
The palette is the five inks and papers above, nothing else. Interactive
...
| 20px / 1.2 to 1.25                | 500    | Card titles, row titles, sub-headings (`h2` in cards, `h3`) |
| 16px / 1.6                        | 400    | Body copy, capped at a 66ch measure                         |
| 13 to 15px / 1.2 to 1.6           | 400    | Nav, form inputs, dense UI text                             |
| 11px mono / 1, +0.14em, uppercase | 500    | Labels (field labels), with the breadcrumb at 12px mono     |
...
`--hairline`. Nothing has radius or shadow. The only fill is ink on
...
| Views pager        | `div[mode] ul li`                                       | Mono 13px in 1px rule boxes. Hover promotes to an ink border.                                                                                                                                |
| Main menu          | `div[name="header"] li a`                               | Active trail marked by an ink bottom border rather than colour.                                                                                                                              |
| Language block     | `div[name="pre_header"] li`                             | Inline mono links, 12px. The pending state renders an empty div so the header doesn't shift.                                                                                                 |
| Reactive filtering | `pages/examples/reactive-filtering.vue` (scoped styles) | The one page with bespoke components. It adds mono chips (active = ink fill), an 8px coverage bar (ink fill on rule track), and a row grid `72px / 1fr / 200px / 96px`.                      |
...
- **`/en/contact`**: `DruxtEntityForm[contact_message--feedback]`, with inputs
  styled generically (label above, 1px rule border, no radius) so any field
  type the form module emits gets the same rhythm.
...
None. The wireframe is a single-column flow, and grids degrade naturally
