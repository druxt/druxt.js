before(() => {
  // Start dev server - components are imported directly so no env var needed
  cy.exec('pkill -f "yarn dev" || true')
  cy.exec('PORT=3000 timeout 15 yarn dev > /dev/null 2>&1 &', { timeout: 10000 })
...
it('Deprecated Field Components: Render and Warnings', () => {
  ...
  cy.get('[data-cy="deprecated-components-container"]', { timeout: 10000 }).should('exist')
  ...
  cy.get('[data-cy="deprecated-components-container"]').within(() => {
    ...
    cy.contains('This is a long text that should be trimmed.').should('be.visible')
  ...
  cy.window().then((win) => {
    ...
    const deprecatedWarnings = warningMessages.filter(w =>
    ...
    expect(deprecatedWarnings.some(w => w.includes('DruxtFieldTextDefault'))).to.be.true
    expect(deprecatedWarnings.some(w => w.includes('DruxtFieldTextTrimmed'))).to.be.true
    expect(deprecatedWarnings.some(w => w.includes('DruxtFieldNumberInteger'))).to.be.true
    expect(deprecatedWarnings.some(w => w.includes('DruxtFieldLink'))).to.be.true
