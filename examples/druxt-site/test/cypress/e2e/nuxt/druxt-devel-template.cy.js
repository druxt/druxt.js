...
it('DruxtDevelTemplate tool', () => {
  ...
  cy.get(searchBlock)
    ...
    .should(
      'contain.text',
      "[DruxtBlock] Missing Vue template for the 'umami_search' block"
    )
    .click()
        ...
        cy.get(searchBlock)
          ...
          .then((options) => {
            ...
            if (values.length > 0) {
              ...
              cy.get(searchBlock).find('select').select(values[0])

              // Check that the button exists
              cy.get(searchBlock).find('button').should('be.visible')
