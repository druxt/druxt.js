<template>
  <div v-if="!$fetchState.pending">
    ...
    <blockquote>
      The DruxtEntity query settings allow including and filtering fields from
    ...
    <dl>
      ...
      <dd>
        <p>
          An array or arrays of fields to filter from the returned resource(s).
        </p>
        <p>
          If an array of strings is provided, they will be used to filter the
          main Entity.
        </p>
  ...
  ['user--user', ['display_name']] // Filter the `user--user` resource fields.
      ...
      <dd>
        <p>
          Boolean; If true, fields will be filtered based on the schema, as well
          as any explicitly defined fields.
        </p>
      </dd>
    </dl>
    ...
    <p>
      The default behaviour can be configured in <code>nuxt.config.js</code>
    </p>
    ...
    <p>
      This example is using the query settings on a Theme component to filter
      just the schema fields.
    </p>
    <p>
      This should print out only properties and relationships requested by the
      Article default schema.
    </p>
  ...
  druxt: {
    ...
    <p>
      This example is using the query settings on a Theme component to filter
      specific fields from the Entity resource.
    </p>
  ...
  druxt: {
    ...
    <p>
      This example is using the query settings on a Theme component to include
      related resources.
    </p>
    <p>
      It's also possible to filter the related resources by using the an array
      of arrays compoatible with the JSON:API Params
      <a href="https://www.npmjs.com/package/drupal-jsonapi-params#addfields"
        >addFields</a
      >
      method.
    </p>
    <p>
      This should show the Recipe fields, including already requested fields,
      and the filtered relationship data.
    </p>
  ...
  druxt: {
    ...
    <p>
      This example applies the query settings as a property of the DruxtEntity
      component.
    </p>
    <p>
      This should show the already requested Page data with the filtered author
      entity included.
    </p>
...
export default {
  async fetch() {
    const uuids = await Promise.all(
      Object.keys(this.uuids).map((type) => {
