...
describe('DruxtClient', () => {
  ...
  test('constructor', () => {
    ...
    expect(new DruxtClient(baseUrl, { axios: { headers } })).toBeInstanceOf(
      DruxtClient
    )
  ...
  test('buildQueryUrl', () => {
    ...
    expect(druxt.buildQueryUrl('url', '?query')).toBe('url?query')
    expect(druxt.buildQueryUrl('url', { getQueryString: () => 'query' })).toBe(
      'url?query'
    )
    expect(druxt.buildQueryUrl('url', { query: 'string' })).toBe(
      'url?query=string'
    )
  ...
  test('getRelated', async () => {
    ...
    expect(mockAxios.get).toHaveBeenCalledWith(
      `${baseUrl}/en/jsonapi/node/article/${id}/field_media_image`,
      undefined
    )
    ...
    try {
      ...
      expect(mockAxios.get).toHaveBeenCalledWith(
        `/jsonapi/node/fake/${id}/field_media_image`,
        undefined
      )
    }
  })
  ...
  test('getResource', async () => {
    const mockArticle = await getMockResource('node--article')
    const entity = await druxt.getResource(
      mockArticle.data.type,
      mockArticle.data.id
    )
    ...
    expect(mockAxios.get).toHaveBeenCalledWith(
      `${baseUrl}/en/jsonapi/node/article/missing`,
      undefined
    )
