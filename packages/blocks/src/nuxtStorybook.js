...
export default async function ({ stories }) {
  ...
  const blocks = (await druxt.getCollectionAll('block--block', query))
    ...
    addTemplate({
      ...
      options: {
        ...
        title: titleFn([
          ...
          block.attributes.drupal_internal__id,
        ]),
      },
    })
  })
  ...
  themes.forEach((theme) => {
    const regions = Array.from(
      new Set(
        blocks
          .filter((o) => o.attributes.theme === theme)
          .map((o) => o.attributes.region)
      )
    )
    regions.forEach((region) => {
      addTemplate({
        src: resolve(
          __dirname,
          `../templates/druxt-block-region.instance.stories.js`
        ),
  ...
  stories.push(
    resolve(options.buildDir, `./stories/druxt-block-region.stories.js`)
  )
  stories.push(resolve(options.buildDir, `./stories/druxt-block.*.stories.js`))
  stories.push(
    resolve(options.buildDir, `./stories/druxt-block-region.*.stories.js`)
