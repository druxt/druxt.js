...
export default async function ({ stories }) {
  ...
  const [view, form] = (
    await Promise.all(
      ['view', 'form'].map(
        async (type) =>
          await druxt.getCollectionAll(
            `entity_${type}_display--entity_${type}_display`,
    ...
    schema.map((collection) => collection.data.map((o) => o)).flat()
  )
  const schemas = [...view, ...form]

  // Get Entity type and bundle data.
  const entityTypes = Array.from(
    new Set(schemas.map((o) => o.attributes.targetEntityType))
  ).map((entity) => ({
    entity,
    bundles: Array.from(
      new Set(
        schemas
          .filter((o) => o.attributes.targetEntityType === entity)
          .map((o) => o.attributes.bundle)
      )
    ),
  }))
        ...
        bundles.map(async (bundle) => {
          try {
            return {
              ...
              entities: (
                await druxt.getCollection(
                  `${entity}--${bundle}`,
                  new DrupalJsonApiParams()
                    .addFilter('status', 1)
                    .addFields(`${entity}--${bundle}`, [
                      'id',
                      'title',
                      'name',
                      'info',
  ...
  stories.push(
    resolve(options.buildDir, `./stories/druxt-entity-form.stories.js`)
  )

  // BuildEntity* Instance Stories.
  entityTypes.forEach(({ entity, bundles }) =>
    bundles.map((bundle) => {
      Object.entries({ view, form }).forEach(([type, schemas]) => {
        const displays = Array.from(
          new Set(
            schemas
              .filter(
                (o) =>
                  o.attributes.targetEntityType === entity &&
                  o.attributes.bundle === bundle
              )
              .map((o) => o.attributes.mode)
        ...
        if (displays) {
          ...
          const component =
            type === 'view' ? 'druxt-entity' : 'druxt-entity-form'

          addTemplate({
            src: resolve(
              __dirname,
              `../templates/${component}.instance.stories.js`
            ),
            fileName: `stories/${component}.${resourceType}.stories.js`,
            options: {
              displays,
              entities: entities.find((o) => o.resourceType === resourceType)
                .entities,
              resourceType,
              title: titleFn([
                'Druxt',
          ...
          stories.push(
            resolve(
              options.buildDir,
              `./stories/${component}.${resourceType}.stories.js`
            )
          )
        }
      })
