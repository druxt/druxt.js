...
`
  ...
  async processVue(file, templateData) {
    ...
    // Props.
    for (const prop of (data.props || []).filter(
      (prop) =>
        !templateData
          .map((o) => o.id)
          .includes(`${prefixId}.props.${prop.name}`)
    )) {
      const item = {
      ...
      if ((prop.tags || {}).example) {
        item.examples = [
          prop.tags.example[0].description.replace(
            'DruxtModule',
            prefixId.split(':')[1]
          ),
        ]
  ...
  writeTemplateData(file, templateData) {
    ...
    const content = dmd(templateData, {
      ...
      'heading-depth': 1,
      partial: [path.resolve(cwd, 'partials/sig-name.hbs')],
      separators: true,
    })
    if (!content) return

    const destination = `${this.destination}/api/${file
      .replace('src/', '')
      .replace(/\.[^/.]+$/, '.md')}`
    mkdirp.sync(path.dirname(destination))
