...
export default async function ({ stories }) {
  ...
  for (const view of collections) {
    ...
    const displays = Object.values(display).filter((item) => {
      const options =
        item.id === 'default' ? item : merge(display.default, item)

      const extenders = options.display_options.display_extenders
      const enabled = !(
        !Array.isArray(extenders) && !(extenders.jsonapi_views || {}).enabled
      )

      return enabled
        ? options.display_options.row.type.startsWith('entity:')
        : false
    })
